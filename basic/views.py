...
def createCircle(request):
    circle_creator = UserRegister.objects.get(username = request.user.userregister)
    if request.method=='POST':
        ...
        form = CircleForm(initial={'circle_creator':circle_creator, 'members':request.user.userregister, 'neighbourhood':request.user.userregister.neighbourhood})
    context = {
        'form':form,
    }
